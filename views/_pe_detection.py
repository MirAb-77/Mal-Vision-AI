...
""", unsafe_allow_html=True)
    ...
    with tab2:
        ...
        v_key = "benign" if variant.startswith("✅") else "malicious"
    ...
    # DEMO MODE FOR DUMMY DATA
            if input_source and input_source.startswith("Dummy"):

                import random

                conf = round(random.uniform(0.60, 0.75), 4)

                if "benign" in input_source.lower():

                    label = "Benign"

                    proba = np.array([
                              conf,          # benign probability
                              1 - conf       # malicious probability
                             ])
                    ...
                    proba = np.array([
                             1 - conf,      # benign probability
                             conf           # malicious probability
                            ])

                mode = "DEMO MODE"

    # REAL MODEL FOR UPLOADED FILES
            else:
                try:
